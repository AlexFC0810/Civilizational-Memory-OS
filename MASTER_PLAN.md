...
| 21c | ③ **PR #60** Prophetic Root — fold Khaldunian + Truth-Desk-80/20 gates into `DEPLOYMENT_READINESS_GATE.md` as upstream review steps (the CIG-adoption path); discovery archive = provenance-of-REASONING, cross-refs CCR (provenance-of-claims), never duplicates it; Truth-Desk *software* stays parked behind creator-value proof | Sonnet binding edits → operator | binding headers added, then merge | 4 | 3 | H1 | **BIND, DON'T DUPLICATE** |
| 21d | ④ **PR #62+#67** mercy pair — file-by-file de-dup to ONE mercy/anti-impunity spine before either leaves draft; #67 memetics doctrine passed truth-boundary check (2026-07-18, no crossing found); its 0–64 scorecard folds as review lens, publication authority stays `evals.mjs`; campaign/distribution → T&P; folds into `mercy-and-the-powerless` pack | Sonnet de-dup map → operator | de-dup map + folded scorecard, then merge together | 4 | 3 | H1–H2 | **DE-DUP, THEN MERGE PAIRED** |
| 21e | ⑤ **PR #68** Mīzān Archive — rename in-fiction "The Archivist" + "canonical claim adjudication" away from the operational seat (`THE_ARCHIVIST.md`, lands with #55) and substrate vocabulary; carries rebase burden for AGENTS.md (∩#57) + README (∩#62); distribution T&P-owned, universe bible stays here as upstream truth-architecture | ChatGPT revision → operator | namespaced fiction + clean rebase, then merge last | 3 | 2 | H2 | **NAMESPACE, MERGE LAST** |
| 21f | ⑥ **PR #70** Truth Sovereignty / reality-audit (triaged 2026-07-19) — protocol file passes structure check (method, not gate); but the AIOS.md hunk writes two "law" sections into the control plane + declares doctrine-hub files "upstream canonical" → route via row 18's `cross_repo_state` truth-contract; planes↔layers vocab map; Wang Wang cross-links #67; transcript archive = provenance-of-reasoning, cross-refs CCR | ChatGPT revision → operator decision on AIOS.md | reconciled per PR comment, then merge after #68 | 3 | 3 | H1–H2 | **BIND GOVERNANCE, DON'T FORK** |
| 22 | **Wave hygiene (2026-07-19 steward pass):** ① direct-to-main canon commit c063a22 (`canon/EPISTEMIC_ARCHITECTURE_AND_KNOWLEDGE_PIPELINE.md`, 889 lines, no PR — first protocol regression since row 20's win) → issue #71: bind its chain-vocabulary to the five planes via a small header PR; no gate authority claimed, keep it that way; check `--intake` delta ② PR #13 kill-rule trip (non-draft 22 days, title says DRAFT) → disposition comment posted: mark-draft now, close-with-absorption once the carding queue registers its claims | Operator clicks; steward tracks | issue #71 closed via header PR; #13 state honest | 3 | 4 | H1 | **PULL — two small clicks** |
